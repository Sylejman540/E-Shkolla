...
try {
    ...
    /* ==========================================================
       LOGJIKA E ARKIVIMIT (Vonesa 24 orë)
       Shtojmë: AND g.created_at < CURDATE()
    ========================================================== */
    $stmt = $pdo->prepare("
    ...
    // Mesatarja llogaritet vetëm mbi notat e arkivuara për konsistencë
    $averageGrade = 0;
    if (count($grades) > 0) {
        $sum = array_sum(array_column($grades, 'grade'));
        $averageGrade = round($sum / count($grades), 2);
    }
...
<div class="max-w-7xl mx-auto space-y-6 pb-10 px-4 text-sm font-normal text-slate-600">
    ...
    <div class="bg-white rounded-[24px] border border-slate-100 shadow-sm p-6 relative overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 relative z-10">
            <div>
                ...
                <p class="text-slate-400 text-xs mt-0.5 italic">Arkiva akademike për: <span class="font-bold text-indigo-500"><?= htmlspecialchars($children[0]['name']) ?></span></p>
            </div>
        ...
        <div class="mt-4 flex gap-1.5">
            <?php foreach ($children as $child): ?>
                <a href="?student_id=<?= $child['student_id'] ?>" 
                   class="px-4 py-1.5 rounded-lg text-[10px] font-bold transition-all <?= $child['student_id'] === $studentId ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-50 text-slate-400 hover:bg-slate-100' ?>">
    ...
    <div class="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
            ...
            <span class="text-[10px] font-bold text-slate-400 uppercase">Totali: <?= count($grades) ?> Nota</span>
        </div>
