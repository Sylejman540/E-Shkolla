...
// Fetch schedule with class name and subject name
$stmt = $pdo->prepare("
    SELECT 
        cs.day, 
        cs.start_time, 
...
<div class="space-y-6">
        ...
        <div class="grid grid-cols-5 divide-x divide-slate-100 min-h-[600px] bg-white">
            <?php for ($i = 1; $i <= 5; $i++): ?>
                <div class="p-3 space-y-3 bg-slate-50/20">
                    <?php 
                    $dailyClasses = array_filter($scheduleItems, fn($item) => $item['day_of_week'] == $i);
                    ...
                    ?>
                        <div class="bg-white border-l-4 border-blue-600 rounded-xl p-3 shadow-sm ring-1 ring-slate-200/60 hover:shadow-md transition-shadow">
                            ...
                            <div class="text-xs text-slate-500 mt-1 flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" stroke-width="2"/></svg>
    ...
    <div class="lg:hidden space-y-6">
        ...
        ?>
            <div class="space-y-3">
                <h3 class="text-sm font-bold text-slate-400 uppercase tracking-widest pl-2"><?= $dayName ?></h3>
                <?php if (empty($dailyClasses)): ?>
                    <div class="bg-slate-100/50 rounded-2xl p-4 text-center text-xs text-slate-400 italic">Pushim</div>
                <?php else: ?>
                    <?php foreach ($dailyClasses as $class): ?>
