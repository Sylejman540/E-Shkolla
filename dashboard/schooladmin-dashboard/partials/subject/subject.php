...
// --- 1. FETCH DATA (Main Query) ---
// Note: LIMIT/OFFSET are injected as integers to satisfy native prepares
$sql = "SELECT 
            s.id,
            s.subject_name, 
...
// --- 2. COUNT TOTAL (Pagination Logic) ---
...
$range = [];
if ($totalPages <= 7) {
    $range = $totalPages > 0 ? range(1, $totalPages) : [];
} else {
...
<div id="subjectTableContainer" class="px-4 sm:px-6 lg:px-8 py-8 transition-opacity duration-200">
    <div class="sm:flex sm:items-center justify-between mb-8">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Menaxhimi i lëndëve dhe statusi i mësuesve përgjegjës.</p>
        </div>
    ...
    <div class="mb-6 bg-white dark:bg-gray-900 p-4 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm flex items-center justify-between">
        <div class="relative w-full max-w-xs flex items-center gap-2">
            ...
            <?php if(!empty($search)): ?>
            <button onclick="clearSearch()" class="p-2.5 rounded-xl bg-slate-100 dark:bg-gray-800 text-slate-500 hover:text-rose-500 transition-colors">
            ...
            <table class="w-full text-left border-collapse table-fixed min-w-[700px]">
                <thead>
                    <tr class="bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/10">
                        <th class="w-[40%] px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Emri i Lëndës</th>
                        <th class="w-[35%] px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Mësimdhënësi Përgjegjës</th>
                        <th class="w-[25%] px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 text-center">Statusi i Mësuesit</th>
                    </tr>
                </thead>
                <tbody id="subjectsTableBody" class="divide-y divide-slate-200 dark:divide-white/5">
                    <?php if(!empty($subjects)): ?>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap overflow-hidden">
                                <span contenteditable class="editable block text-sm font-bold text-slate-900 dark:text-white outline-none truncate px-1 rounded hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition" 
                                      data-id="<?= $row['id'] ?>" data-field="subject_name" data-original="<?= htmlspecialchars($row['subject_name']) ?>">
                            ...
                            <td class="px-6 py-4 text-center">
                                <?php if($row['teacher_display_name']): ?>
                                    <button class="status-toggle px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all <?= $row['teacher_display_status'] === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : 'bg-red-100 text-red-600 dark:bg-red-500/20 dark:text-red-400' ?>" 
                                            data-id="<?= $row['teacher_user_id'] ?>" data-field="status" data-value="<?= $row['teacher_display_status'] ?>" data-original="<?= $row['teacher_display_status'] ?>" data-target="teacher">
                                        <?= htmlspecialchars($row['teacher_display_status']) ?>
                                    </button>
                                <?php else: ?>
                                    <span class="text-slate-400 italic text-xs tracking-tight">Pa mësues</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="3" class="py-12 text-center text-sm text-slate-500 italic">Nuk u gjet asnjë lëndë.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
async function save(el, forcedValue = null) {
    ...
    try {
        const response = await fetch(targetURL, {
            ...
            body: JSON.stringify({ [isTeacher ? 'userId' : 'subjectId']: id, field, value: newValue })
        });
...
document.getElementById('confirmStatus').onclick = () => {
    if (window.pendingChange) save(window.pendingChange.btn, window.pendingChange.newStatus);
