...
// Merr mësuesit për kutinë e ndihmës
...
// Gjenero vitin akademik automatik
$currentMonth = date('n');
$currentYear = date('Y');
...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    <div class="sm:flex sm:items-center justify-between mb-8">
        ...
        <div class="flex gap-3">
            ...
            <button onclick="downloadClassTemplate()" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl hover:bg-emerald-100 transition-all shadow-sm">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                Shkarko Modelin
            </button>
            <a href="/E-Shkolla/classes" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all shadow-sm">
                Anulo
            </a>
        </div>
    </div>
    ...
    <div id="helpBox" class="hidden mb-6 p-6 bg-slate-800 text-white rounded-2xl shadow-xl border border-slate-700">
        ...
        <p class="mt-4 text-xs text-slate-400">Viti akademik aktual: <b><?= $autoYear ?></b>. Statusi: 'active' ose 'inactive'.</p>
    </div>
    ...
    <div id="uploadContainer" class="bg-white dark:bg-gray-900 rounded-3xl border-2 border-dashed border-slate-200 dark:border-white/10 p-12 text-center shadow-sm transition-all hover:border-indigo-400">
        <div class="max-w-md mx-auto">
            ...
            <p class="text-sm text-slate-500 mb-6 italic">Sigurohuni që kolonat 'academic_year' dhe 'grade' janë të plotësuara.</p>
    ...
    <div id="previewContainer" class="hidden space-y-6">
        ...
        <div class="flex items-center justify-between p-4 bg-emerald-50 dark:bg-emerald-500/5 rounded-2xl border border-emerald-100 dark:border-emerald-500/20">
            <p class="text-sm text-emerald-700 dark:text-emerald-300 px-2 font-medium">
                Kontrolloni të dhënat më lart. Klasat që ekzistojnë do të anashkalohen automatikisht.
            </p>
            <div class="flex gap-3">
    ...
    Papa.parse(file, {
        ...
        complete: function(results) {
            classData = results.data
                ...
                .filter(row => row.academic_year && row.grade);

            if (!classData.length) {
                alert('Gabim: Skedari nuk ka të dhëna valide. Sigurohuni që kolona "grade" nuk është bosh.');
                return;
            }
...
function normalizeClassRow(row) {
    ...
    return {
        academic_year: clean.academic_year || clean.academic_grade || '<?= $autoYear ?>',
        grade: clean.grade || clean.klasa || clean.academic_grade || '', 
        teacher_id: clean.teacher_id || null,
        max_students: clean.max_students || clean.max_stude || 30,
...
document.getElementById('submitImportBtn').onclick = async function () {
    ...
    try {
        // SIGUROHUNI që ky path është saktësisht ku ndodhet skedari juaj PHP
        const response = await fetch('/E-Shkolla/dashboard/schooladmin-dashboard/partials/classes/process-import.php', {
            method: 'POST',
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                alert("Gabim nga serveri: " + result.message);
            }
        } catch (jsonErr) {
            console.error("Server Output:", text);
            alert("Gabim: Serveri nuk ktheu JSON të rregullt. Kontrolloni Console (F12).");
        }
    } catch (e) {
        console.error("Fetch Error:", e);
        alert("Gabim rrjeti: Nuk u gjet skedari process-import.php");
    } finally {
        btn.disabled = false;
...
function downloadClassTemplate() {
    ...
    const sampleData = ["<?= $autoYear ?>", "10", "1", "30", "active"];
