...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center justify-between">
        <div class="mt-5">
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Nxënësit e Shkollës</h1>
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Menaxhoni të dhënat e nxënësve në kohë reale.</p>
        </div>
    ...
    <div class="mt-6 mb-4 flex items-center gap-2">
        <div class="relative w-full sm:w-64">
            <input id="liveSearch" type="text" placeholder="Kërko nxënësit..." class="w-full pl-10 pr-4 py-2 rounded-lg border text-sm dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" oninput="filterStudents()">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition">
                            <td class="py-4 pl-4 pr-3 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white sm:pl-0">
                                <span contenteditable class="editable inline-block min-w-[10rem] px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="name" data-original="<?= htmlspecialchars($row['name']) ?>">
                            ...
                            <td class="px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <span contenteditable class="editable px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="class_name" data-original="<?= htmlspecialchars($row['class_name']) ?>">
                                    <?= htmlspecialchars($row['class_name']) ?>
                                </span>
                            </td>
                            <td class="px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <span contenteditable class="editable px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="date_birth" data-original="<?= htmlspecialchars($row['date_birth']) ?>">
                                    <?= htmlspecialchars($row['date_birth']) ?>
                                </span>
                            </td>
                            <td class="px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <span contenteditable class="editable px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="email" data-original="<?= htmlspecialchars($row['email']) ?>">
                            ...
                            <td class="py-4 text-right text-sm font-medium">
                                <a href="/E-Shkolla/parents?student_id=<?= (int)$row['student_id'] ?>&open_form=1" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 font-semibold">Shto prindër</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
...
<div id="statusModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-xl border border-gray-200 dark:border-white/10">
        <div class="text-center">
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">A jeni të sigurt? Kjo do të ndikojë në aksesin e nxënësit.</p>
        </div>
