...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center justify-between">
        <div class="mt-5">
            <h1 class="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Mësuesit e Shkollës</h1>
            <p class="mt-2 text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400">Menaxhoni listën e mësuesve dhe lëndët e tyre.</p>
        </div>
    ...
    <div class="mt-6 mb-4 flex items-center gap-2">
        <div class="relative w-full sm:w-64">
            <input id="liveSearch" type="text" placeholder="Kërko mësuesit..." class="w-full pl-10 pr-4 py-2 rounded-lg border text-sm dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" oninput="filterTeachers()">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition">
                            ...
                            <td class="px-3 py-4 text-sm font-medium text-gray-900 dark:text-white">
                                <span contenteditable class="editable inline-block min-w-[8rem] px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="name" data-original="<?= htmlspecialchars($row['name']) ?>"><?= htmlspecialchars($row['name']) ?></span>
                            </td>
                            <td class="px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <span contenteditable class="editable inline-block min-w-[8rem] px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="email" data-original="<?= htmlspecialchars($row['email']) ?>"><?= htmlspecialchars($row['email']) ?></span>
                            </td>
                            <td class="px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <span contenteditable class="editable inline-block min-w-[8rem] px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="phone" data-original="<?= htmlspecialchars($row['phone']) ?>"><?= htmlspecialchars($row['phone']) ?></span>
                            </td>
                            <td class="px-3 py-4 text-sm">
                                <select class="editable-select rounded-full px-3 py-1 text-xs border bg-gray-50 dark:bg-gray-800 dark:text-white appearance-none" data-id="<?= $row['user_id'] ?>" data-field="gender" data-original="<?= $row['gender'] ?>">
                            ...
                            <td class="px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <span contenteditable class="editable inline-block min-w-[8rem] px-2 py-1 rounded outline-none" data-id="<?= $row['user_id'] ?>" data-field="subject_name" data-original="<?= htmlspecialchars($row['subject_name']) ?>"><?= htmlspecialchars($row['subject_name']) ?></span>
                            </td>
                            <td class="px-3 py-4 text-sm">
...
<div id="statusModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-xl border border-gray-200 dark:border-white/10">
        <div class="text-center">
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Ky veprim do të ndikojë në qasjen e mësuesit në ditarin elektronik.</p>
        </div>
