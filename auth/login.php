...
================================ */
// If user isn't logged in but has the cookie, log them in automatically
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (!isset($_SESSION[$lock_key])) {
            $_SESSION[$lock_key] = ['count' => 0, 'locked_until' => 0];
        }
        
        if ($_SESSION[$lock_key]['locked_until'] >= time()) {
            $remaining = ceil(($_SESSION[$lock_key]['locked_until'] - time()) / 60);
            $error = "Shumë tentime. Provoni përsëri pas {$remaining} minutave.";
        } else {
            if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if ($user && password_verify($password, $user['password'])) {
                    ...
                    if ($_SESSION[$lock_key]['count'] >= 5) {
                        $_SESSION[$lock_key]['locked_until'] = time() + 900;
                    }
    ...
    <div class="mx-auto w-full max-w-md">
      ...
      <div class="mb-10">
        <div class="mb-6 flex items-center">
          <div class="flex h-20 w-20 items-center justify-start">
      ...
      <div class="mb-4 rounded-lg bg-emerald-50 p-4 text-sm text-emerald-800 border border-emerald-200" role="alert">
          Ju keni dalë me sukses.
      </div>
      ...
      <form method="POST" class="space-y-5">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Adresa e email-it</label>
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button type="button" id="togglePassword" class="absolute inset-y-0 right-0 flex items-center px-3 text-gray-500 hover:text-blue-600 dark:text-gray-400">
        ...
        <div class="flex items-center">
            ...
            <label for="remember_me" class="ml-2 block text-sm text-gray-700 dark:text-gray-300 font-medium">Më mbaj mend për 30 ditë</label>
        </div>
...
<script>
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    if (isPassword) {
      eyeIcon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7a9.956 9.956 0 012.362-4.162M6.423 6.423A9.956 9.956 0 0112 5c4.478 0 8.268 2.943 9.542 7a9.978 9.978 0 01-4.043 5.362M15 12a3 3 0 00-3-3M3 3l18 18" />`;
    } else {
      eyeIcon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>`;
    }
  });
</script>
</body>
